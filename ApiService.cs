...
public static class ApiService {
    ...
    public static async Task Start(CancellationToken cancellationToken) {
        ...
        while (_run) {
            ...
            RateLimiter.NewRequest(request.RemoteEndPoint.Address);
            if (RateLimiter.CheckUser(request.RemoteEndPoint.Address)) {
                statusCode = 429;
                responseString = $"{{ \"Error\":\"Too Many Requests, {RateLimiter.GetInfoString(request.RemoteEndPoint.Address)}\" }}";
            }
            else {
                try {
